...
Before adding this **regular expression** to the previously created **Regular Expression Extractor**, I can **test it beforehand** by using the **Search** and **Find** functions with the **Regular exp. option enabled** in **View Results Tree**. This **regular expression** can indeed find the **string I am looking for**, but there is one **caveat** with the **Find** function in **View Results Tree**: it **does not highlight the capture group**, which is the **part of the match that will be extracted**. But, I am quite certain **this regular expression will work in practice**.
