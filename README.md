...
Now when I run the script, all the **Response Assertions** fail, which is actually surprising because it means even the **first page** (https://authenticationtest.com/) in my **test case** is **not returning** the expected, readable **Response Body**. I open **View Results Tree**, select the **HTTP Request** that **loads the first page**, and select **Response Body** under the **Response data** tab. For some reason, I was unable to copy all of the content in the **Response Body**, but it just looks like more of these symbols:
